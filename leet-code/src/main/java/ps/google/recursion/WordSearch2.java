...
public class WordSearch2 {
    public List<String> findWords(char[][] board, String[] words) {
        ...
        for(int i = 0; i < board.length; i++) {
            for(int j = 0; j < board[0].length; j++) {
                dfs(i, j, 0, board, new HashSet<>(), dict, seen);
            }
        }
        ans.addAll(seen);
        return ans;
    ...
    private void dfs(int i, int j, final int pos, char[][] board, Set<String> visited, Set<String> dict, Set<String> seen) {
        if(dict.size() == 0)
            return;
        ...
        for(String di : dict) {
           if(di.length() == pos + 1 && di.charAt(pos) == bchar) {
               seen.add(di); //positive
           } else if(di.length() <= pos || di.charAt(pos) != bchar) {
               negative.add(di); //negative
           }
        }
        dict.removeAll(seen);
        dict.removeAll(negative);
        //now dict has all the candidates
        visited.add(i+","+j);
        dfs(i + 1, j, pos + 1, board, visited, dict, seen);
        dfs(i - 1, j, pos + 1, board, visited, dict, seen);
        dfs(i , j + 1, pos + 1, board, visited, dict, seen);
        dfs(i , j - 1, pos + 1, board, visited, dict, seen);
        dict.addAll(negative);
        visited.remove(i+","+j);
        //restore dict to the previous state;
    }
